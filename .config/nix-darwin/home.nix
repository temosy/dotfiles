...

{
  ...
  home.packages = [
    ...
    # system-kakari のバックアップ用 S3（東京 ap-northeast-1 / 大阪 ap-northeast-3）の
    # 作成と確認に使う。**バックアップ本体はサーバー側の boto3 が実行する**ので、
    # ここに入れるのは Mac から設定・検証するため（バケット作成、公開遮断の確認、
    # 復元テストでのダウンロード）。
    # 認証情報は ~/.aws に置かず 1Password の automation ボールトから
    # `op read` で環境変数に流し込む（値をディスクにもシェル履歴にも残さない）。
    pkgs.awscli2

    # S3 バックアップの**復号**に使う。サーバー側は gpg 2.4.9 が入っているが、
    # Mac に無いと「サーバーが全損したときに手元で戻せない」ことになり、
    # バックアップの意味が消える（2026-08-04 の復元テストで実際に詰まった）。
    pkgs.gnupg
  ];
